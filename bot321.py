...
DB_TABLE_ID = "11KbeilP1HRonHQAAZusBS1-ffNo4FxHXa239yZMKJm8" # Твоя база данных
OWNER_ID = 879365319 
...
# --- ФУНКЦИИ БАЗЫ ДАННЫХ (Через API) ---
async def get_all_users():
    """Получает список всех ID из Google Таблицы"""
    url = f"https://sheets.googleapis.com/v4/spreadsheets/{DB_TABLE_ID}/values/Sheet1!A:A?key={GOOGLE_API_KEY}"
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as resp:
            data = await resp.json()
            rows = data.get("values", [])
            return [row[0] for row in rows if row and row[0].isdigit()]

# --- ФУНКЦИИ ПАРСИНГА (СТУДЕНТЫ/УЧИТЕЛЯ) ---
...
async def fetch_student_schedule(course, group, target_day=None):
    ...
    for r in range(min(5, len(rows))):
        for i, cell in enumerate(rows[r]):
            if group.lower() in cell.lower(): col_idx = i; break
        if col_idx != -1: break
    ...
    for i in range(len(rows)):
        ...
        if pair_num and content and content not in ["-", ".", "№", "Ден"]:
            ...
            if i + 1 < len(rows) and len(rows[i+1]) > col_idx:
                t_val = rows[i+1][col_idx].strip(); teacher = f" ({t_val})" if t_val else ""
...
@dp.callback_query(основнойdata == "admin_stats")
...
async def broadcast_step2(message: types.Message, state: FSMContext):
    await state.clear()
    users = await get_all_users()
    await message.answer(f"🚀 Начинаю отправку на {len(users)} чел...")
